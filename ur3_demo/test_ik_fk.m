function res_T = test_ik_fk(joint_angles)
    addpath(genpath('ur3_utils'),genpath('ur3_robot'),genpath('ur3_inverse_kinematics'),genpath('ur3_forward_kinematics'))
    ...
    % 正解笛卡尔坐标位置与姿态
    cartesian_position_DH = ur3_forward_kinematics(6,joint_angles)

    
    % 笛卡尔坐标位置与姿态逆解关节角度
    %[joints1,joints2,joints3,joints4,joints5,joints6,joints7,joints8] = inverse_kinematics(cartesian_position_DH);
    [valid,result_list] = ur3_inverse_kinematics(cartesian_position_DH);
    ...
    if valid
        ...
        % 这8个关节角度的正解应该和刚刚的相同
        compare_same_xyz(ur3_forward_kinematics(6,joint_set1),cartesian_position_DH,4,4,'joints1 fk',precision);
        compare_same_xyz(ur3_forward_kinematics(6,joint_set2),cartesian_position_DH,4,4,'joints2 fk',precision);
        compare_same_xyz(ur3_forward_kinematics(6,joint_set3),cartesian_position_DH,4,4,'joints3 fk',precision);
        compare_same_xyz(ur3_forward_kinematics(6,joint_set4),cartesian_position_DH,4,4,'joints4 fk',precision);
        compare_same_xyz(ur3_forward_kinematics(6,joint_set5),cartesian_position_DH,4,4,'joints5 fk',precision);
        compare_same_xyz(ur3_forward_kinematics(6,joint_set6),cartesian_position_DH,4,4,'joints6 fk',precision);
        compare_same_xyz(ur3_forward_kinematics(6,joint_set7),cartesian_position_DH,4,4,'joints7 fk',precision);
        compare_same_xyz(ur3_forward_kinematics(6,joint_set8),cartesian_position_DH,4,4,'joints8 fk',precision);
