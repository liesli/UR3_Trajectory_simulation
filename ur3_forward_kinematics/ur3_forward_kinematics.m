function T = ur3_forward_kinematics(i,joint_rads)
% i 是关节, 如果要末端关节就是6
...
function T = fk_single_transform(i,joint_rads)
    ...
    %% general 
    if i==1
            T =[cos(th(i))  -sin(th(i))*cos(alpha_0)      sin(th(i))*sin(alpha_0)     a_0*cos(th(i)); 
            sin(th(i))     cos(alpha_0)*cos(th(i))      -cos(th(i))*sin(alpha_0)       a_0*sin(th(i));
            0                    sin(alpha_0)               cos(alpha_0)                      d(1);
            0                           0                           0                          1];
    else
            T =[cos(th(i))  -sin(th(i))*cos(alpha(i-1))      sin(th(i))*sin(alpha(i-1))      a(i-1)*cos(th(i)); 
            sin(th(i))     cos(alpha(i-1))*cos(th(i))      -cos(th(i))*sin(alpha(i-1))       a(i-1)*sin(th(i));
